...
<?php
    $res_stat = $utilisateursController->getStatistics();
    $res_length = $utilisateursController->getLength();
...
<script>
    Morris.Donut({
        ...
        data: [
            <?php foreach($res_stat as $stat): ?>
                { label: "<?php echo $stat['sexe']; ?>", value: <?php echo $stat['nbr']; ?> /<?php echo $res_length['length']; ?> },
            <?php endforeach ?>
        ],
        labelColor: '#000000',
        colors: [
