<?php
	...
	class PlatC {
		...
		function echange($plat){

            $sql = "INSERT INTO plat (Nomplat, descP,categorie, img ,echange ,num)
                      VALUES (:Nomplat, :descP, :categorie, :img ,:echange ,:num)";
         $db = config::getConnexion();
         try{
             $query = $db->prepare($sql);
             $query->execute([
